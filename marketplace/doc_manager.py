...
import time
import hashlib
from marketplace.doc_system.generator import generate_aieos
from marketplace.doc_system.validator import validate_aieos_system, build_aieos_readme
from marketplace.doc_system.doctor import run_diagnostics
...
def install_profile_or_capability(target_name):
    ...
    for key, spec in AIEOS_REGISTRY.items():
        if spec["type"] == "PROFILE":
            ...
            if profile_key_norm == norm or (norm == "researcher" and profile_key_norm in ["quantitativeresearcher", "datascientist"]):
                target_key = key
                break
    ...
    if not target_key:
        ...
        print("Available profiles: software-engineer, quantitative-researcher, data-scientist, startup-founder, psychologist")
