...
""".replace("{output_root_url}", output_root.replace('\\', '/'))
    ...
    for key in sorted(AIEOS_REGISTRY.keys()):
        ...
        if doc_type == "CONSTITUTION":
            ...
            rel_path = f"CAPABILITIES/{category}/{cap_name}/Contract.md"
            link = f"[{spec['name']} Contract](file:///{output_root.replace('\\', '/')}/{rel_path})"
...
def validate_aieos_system(output_root):
    ...
    for key, spec in AIEOS_REGISTRY.items():
        ...
        if doc_type == "CONSTITUTION":
            ...
            files_to_check = ["Contract.md", "Interfaces.md", "Responsibilities.md", "Metrics.md", "Benchmarks.md", "FailureModes.md", "Evolution.md"]
            for f_name in files_to_check:
                p = os.path.join(cap_dir, f_name)
                if not os.path.exists(p):
                    errors.append(f"Missing capability file: {p} is required under contract specification.")
                    
            manifest_path = os.path.join(cap_dir, "manifest.yaml")
            if not os.path.exists(manifest_path):
                errors.append(f"Missing capability manifest: {manifest_path} is required.")
            else:
                ...
                for line in lines:
                    if ":" in line:
                        parts = line.split(":", 1)
                        manifest_data[parts[0].strip()] = parts[1].strip()
                if not manifest_data.get("name") or not manifest_data.get("version"):
                    errors.append(f"Manifest anomaly: {manifest_path} lacks name/version attributes.")
                if manifest_data.get("maturity") not in ["Draft", "Experimental", "Validated", "Production", "Optimized", "Autonomous"]:
                    errors.append(f"Manifest anomaly: {manifest_path} contains invalid maturity level '{manifest_data.get('maturity')}'")
